...
import MoLovModal from './MoLovModal';
...
const BatchReport = ({ history }) => {
  ...
  async function handleSearch(flag) {
    ...
    if (loginDS.current.get('reportType') === 'MO') {
      if (queryDS.current.get('moNum')) {
        if (reportList.findIndex((i) => i.moNum === queryDS.current.get('moNum')) > -1) return;
        params = {
      ...
      } else if (queryDS.current.get('moIdList')) {
        params = {
          moNum: undefined,
          moIdList: queryDS.current.get('moIdList'),
          operationId: queryDS.current.get('operationId'),
          organizationId: loginDS.current.get('orgId'),
          taskTypeCode: 'OPERATION_TASK',
        };
    ...
    if (getResponse(res) && res.content) {
      res.content.forEach((i) => {
        ...
        if (!isEmpty(rule) && rule.report_type) {
          ...
          if (rArr.includes('OK')) {
            i.showinputArr.push('ok');
          }
          if (rArr.includes('NG')) {
            i.showinputArr.push('ng');
          }
  ...
  async function handleSubmit() {
    ...
    let flag = true;
    checkList.forEach((i) => {
      if (i.submitOutputLineVoList || i.executableQty > 0) {
        if (!i.submitOutputLineVoList) {
          i.submitOutputLineVoList = [{ pendingQty: i.executableQty }];
        }
        ...
        <div className={styles['query-area']}>
          ...
          {loginDS.current.get('reportType') === 'MO' && (
            <div style={{ display: 'flex', alignItems: 'center' }}>
              ...
              >
                <MoLovModal dataSet={queryDS} onChange={handleSearch} orgObj={orgObj} />
              </div>
