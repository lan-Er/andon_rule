...
export default ({ match, history, location }) => {
  ...
  useEffect(() => {
    ...
    async function defaultLovSetting() {
      ...
      if (getResponse(resp)) {
        detailDS.current.set('moTypeObj', resp.content[0]);
        setMoTypeObj({
          ...resp.content[0],
        });
      }
  ...
  function handleModalCancel() {
    const pathname = `/grwl/mo-workspace/create`;
  ...
  function handleCopy() {
    ...
    history.push({
      pathname: '/grwl/mo-workspace/create',
  ...
  async function handleSave(flag) {
    ...
    if (res && res.content && res.content[0]) {
      // 新建页面创建数据成功后跳转到详情页面
      const pathname = `/grwl/mo-workspace/detail/${res.content[0].ownerOrganizationId}/${res.content[0].moId}`;
      history.push(pathname);
      setChangeFlag(false);
  ...
  return (
    <>
      <Header
        ...
        backPath="/grwl/mo-workspace/list"
        isChange={isChange}
      >
        <Button
          icon="add"
