...
import { PrintDS as printDs } from 'hzero-front-hrpt/lib/stores/labelTemplateDS';
import styles from './index.less';

const Print = (props) => {
  const { location = {} } = props;
  const { search = {}, backPath = '/' } = location;
  const { tenantId, shipOrderNum, requestNum } = queryString.parse(search);
  const { Option } = Select;
      ...
      .then((res) => {
        if (res) {
          ...
          if (res && res.formElements) {
            ...
            if (shipOrderNum) {
              params = {
                [res.formElements[0].name]: shipOrderNum,
              };
            }
            if (requestNum) {
              params = {
                [res.formElements[0].name]: requestNum,
              };
            }
            PrintDS.create(params);
          }
        }
      })
      .catch((err) => {
