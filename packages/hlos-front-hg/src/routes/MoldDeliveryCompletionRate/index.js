...
import { ListDS } from '@/stores/moldDeliveryCompletionRateDS';

const preCode = 'hg.moldDeliveryCompletionRate';
...
@formatterCollections({
  code: ['hg.moldDeliveryCompletionRate', 'hg.common'],
})
export default class MoldDeliveryCompletionRate extends Component {
  get columns() {
    return [
  ...
  handleShowChart(type) {
    ...
    dispatch(
      routerRedux.push({
        pathname: `/hg/mold-delivery-completion-rate-report/chart/${type}`,
        state: params,
      })
    );
  }
  ...
  render() {
    return (
      <Fragment>
        <Header title={intl.get(`${preCode}.view.title.index`).d('模具交期达成率报表')} />
        <Content>
