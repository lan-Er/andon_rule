...
import { Lov, NumberField, Select, TextField, Modal, Button } from 'choerodon-ui/pro';
...
const ByProduction = ({
  ...
  function handleLotEnterDDown(e) {
    if (e.target.value.trim()) {
      ...
      dispatch({
        type: 'taskReport/updateState',
        payload: {
          byProductLotList: JSON.parse(JSON.stringify(_lotArr)),
        },
      });
  ...
  function handleHisOk() {
    dispatch({
      type: 'taskReport/updateState',
      payload: {
        byProductLotList: JSON.parse(JSON.stringify(lotArr)),
      },
    });
    if (lotArr.length) {
  ...
  function handleOk() {
    ...
    if (!list.every((i) => i.executeQty > 0)) {
      notification.warning({
    ...
    <div>
      ...
      <div className={styles['input-area']}>
        {(itemInfo?.itemControlType === 'TAG' || itemInfo?.itemControlType === 'LOT') &&
          !makeLotNumber && (
            <NumberField dataSet={ds} step={0.000001} name="quantity" placeholder="请输入数量" />
          )}
...
export default connect(({ taskReport }) => ({
  lotList: taskReport?.byProductLotList || [],
}))(ByProduction);
