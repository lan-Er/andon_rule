...
import { QueryDS, TaskDS, HeaderDS, ByProductionDS } from '@/stores/taskReportDS';
import {
  checkControlType,
  queryMo,
  runTask,
...
const bDS = () => new DataSet(ByProductionDS());

const TaskReport = ({ history, dispatch, byProductLotList }) => {
  let modal = null;
  ...
  const byProductDS = useDataSet(bDS);
  ...
  const [byProductArr, setByProductArr] = useState([]);
  const [itemInfo, setItemInfo] = useState({});
  const [byProdTagArr, setByProdTagArr] = useState([]);
  ...
  useEffect(() => {
    if (showByInputArr && showByInputArr.length) {
      const inputArr = [];
      showByInputArr.forEach((i) => {
        let item = {};
        if (i === 'ok') {
          item = {
            style: 'OK',
  ...
  function handleTagInput(e) {
    ...
    const inputQty = taskDS.current.get('defaultQty') || 0;
      ...
      if (res && res.content) {
        if (res.content.length > 1) {
          taskModal = Modal.open({
            key: modalKey,
            title: '任务列表',
            className: styles['lmes-task-report-draw-modal'],
            children: (
              <div>
                {res.content.map((i) => {
                  return (
  ...
  async function queryItem(taskId) {
    ...
    if (res && res.content && res.content[0]) {
      ...
      byProductDS.current.set('itemObj', {
        itemId,
        itemCode,
        itemDescription,
      });
      if (inventoryWarehouseId) {
        byProductDS.current.set('warehouseObj', {
      ...
      if (inventoryWmAreaId) {
        byProductDS.current.set('wmAreaObj', {
  ...
  async function handleSelectTask(rec) {
    ...
    const priceRes = await getLastestWorkPrice({
      organizationId: orgObj.organizationId,
    ...
    if (byProductDS.current) {
      byProductDS.current.reset();
    }
    byProductDS.current.set('taskId', rec.taskId);
    queryItem(rec.taskId);
    setCurrentTask(rec);
    ...
    if (!isEmpty(rule)) {
      ...
      if (rule.inspection_report) {
        ...
        if (iArr.includes('PQC.FIRST')) {
          _showFooterBtnArr.push('PQC.FIRST');
        }
        if (iArr.includes('PQC.FINISH')) {
          _showFooterBtnArr.push('PQC.FINISH');
        }
        if (iArr.includes('PQC.LAST')) {
          _showFooterBtnArr.push('PQC.LAST');
        }
      }
    } else {
      _showinputArr.push('ok');
  ...
  async function requestData() {
    ...
    if (getResponse(taskRes) && taskRes.content) {
  ...
  async function handleSubmit() {
    ...
    if (!isEmpty(itemInfo)) {
      const dom = document.getElementsByClassName(styles['lmes-task-report'])[0];
      const byProdValidate = await byProductDS.validate(false, false);
      if (!byProdValidate) {
      ...
      if (itemInfo?.itemControlType === 'LOT' && !lotList.length && !makeLotNumber) {
        notification.warning({
          message: '请录入副产品报工批次后再提交',
        });
        dom.scrollTop = dom.scrollHeight;
        return;
      } else if (itemInfo?.itemControlType === 'TAG' && !byProdTagArr.length) {
        notification.warning({
          message: '请录入副产品报工标签后再提交',
        });
        dom.scrollTop = dom.scrollHeight;
        return;
      }
    }
    const byProdList = getByProdSubmitList();
    if (!byProdList.every((i) => i.executeQty > 0)) {
      notification.warning({
        message: '副产品不合格数量必须大于0',
      });
      return;
    }
    ...
    if (getResponse(res)) {
      ...
      if (!isEmpty(itemInfo)) {
        const {
          ...
          wmAreaCode,
          remark: byProdRemark,
        } = byProductDS.current.toJSONData();
        const wmObj = {
          warehouseId,
          warehouseCode,
          wmAreaId,
        ...
        handleByProductModalOk(itemInfo, byProdList, wmObj);
      }
    }
  }
  ...
  function handleRejectedMaterial() {
    ...
    _rejectedMaterialModal = Modal.open({
      ...
      title: '任务退料',
      className: styles['lmes-task-report-rejected-material-modal'],
      footer: null,
      closable: true,
      destroyOnClose: true,
      style: {
  ...
  async function handleFeeding() {
    ...
    feedingModal = Modal.open({
      key: 'feeding',
      title: '投料',
      className: styles['lmes-task-report-feeding-modal'],
      movable: true,
      closable: true,
      footer: null,
      children: (
  ...
  function handleReport() {
    reportModal = Modal.open({
      key: 'feeding',
      title: '报检',
      className: styles['lmes-task-report-report-modal'],
      movable: true,
      closable: true,
      footer: null,
      children: (
  ...
  function handleByProduct() {
    ...
    productModal = Modal.open({
      key: 'byProduct',
      title: '副产品报工',
      className: styles['lmes-task-report-by-product-modal'],
      footer: null,
      children: (
        <ByProductModal
          taskInfo={taskInfo}
  ...
  function getByProdSubmitList() {
    const list = [];
    if (itemInfo?.itemControlType === 'QUANTITY') {
      if (showMainUom) {
        list.push({
          executeQty: byProductDS.current.get('processOkQty'),
          executeNgQty: byProductDS.current.get('processNgQty'),
          scrappedQty: byProductDS.current.get('scrappedQty'),
          reworkQty: byProductDS.current.get('reworkQty'),
          pendingQty: byProductDS.current.get('pendingQty'),
        });
      } else {
        list.push({
          executeQty: byProductDS.current.get('processOkQty') / taskInfo.uomConversionValue,
          executeNgQty: byProductDS.current.get('processNgQty') / taskInfo.uomConversionValue,
          scrappedQty: byProductDS.current.get('scrappedQty') / taskInfo.uomConversionValue,
          reworkQty: byProductDS.current.get('reworkQty') / taskInfo.uomConversionValue,
          pendingQty: byProductDS.current.get('pendingQty') / taskInfo.uomConversionValue,
        });
      }
    } else if (itemInfo?.itemControlType === 'LOT') {
      byProductLotList.forEach((item) => {
        const processOkQty = makeLotNumber
          ? byProductDS.current.get('processOkQty')
          : item.processOkQty;
        const processNgQty = makeLotNumber
          ? byProductDS.current.get('processNgQty')
          : item.processNgQty;
        const scrappedQty = makeLotNumber
          ? byProductDS.current.get('scrappedQty')
          : item.scrappedQty;
        const reworkQty = makeLotNumber ? byProductDS.current.get('reworkQty') : item.reworkQty;
        const pendingQty = makeLotNumber ? byProductDS.current.get('pendingQty') : item.pendingQty;
        if (showMainUom) {
          list.push({
            lotNumber: item.lotNumber,
  ...
  async function handleByProductModalOk(itemObj, list, wmObj) {
    const paramsArr = [
      {
        organizationId: orgObj.organizationId,
        organizationCode: orgObj.organizationCode,
        itemControlType: itemObj.itemControlType,
  ...
  return (
    <div className={styles['lmes-task-report']}>
      <div className={styles['lmes-task-report-header']}>
      ...
      />
      <div
        className={`${styles['lmes-task-report-content']} ${
          isEmpty(itemInfo) && styles['lmes-task-report-content-margin-bottom']
        }`}
      >
        {!isEmpty(taskInfo) && (
          <ReportInfo
      ...
      {!isEmpty(taskInfo) && !isEmpty(itemInfo) && (
        <div
          className={`${styles['lmes-task-report-by-product-modal']} ${styles['lmes-task-report-by-product']}`}
        >
          <h2>副产品报工</h2>
          <ByProductModal
            taskInfo={taskInfo}
...
export default connect(({ taskReport }) => ({
  byProductLotList: taskReport?.byProductLotList || [],
}))(
