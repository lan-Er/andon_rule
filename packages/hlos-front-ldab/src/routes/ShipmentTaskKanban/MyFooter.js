 ...
 * @LastEditTime: 2020-11-24 14:06:14
            ...
            >
              {newFooterLeft &&
                newFooterLeft.map((item) => {
                  return (
                    <li key={item.shipOrderNum}>
                      <span>
                        {item.customerRank === 'A' ? <img src={keyUserImg} alt="关键" /> : null}{' '}
                        {item.customerName}
                      </span>
                      <span>{item.shipOrderNum}</span>
                      <span>{item.customerAddress}</span>
                      <span>
                        {item.planShipDate ? moment(item.planShipDate).format('YYYY-MM-DD') : ''}
                      </span>
                    </li>
                  );
                })}
              ...
              newFooterRight.map((item) => {
                return (
                  <li key={item.key}>
                    ...
                    <span>{item.executeOrderQty}</span>
                  </li>
                );
              })}
          </div>
