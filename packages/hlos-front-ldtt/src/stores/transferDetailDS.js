...
const preCode = 'ldtf.transfer.detail.model';
const commonCode = 'ldtf.common.model';
const { ldttTransferService } = codeConfig.code;
const url = `${HLOS_LDTF}/v1/transfer-details`;
    ...
    queryFields: [
      {
        name: 'serviceCode',
        type: 'string',
        label: intl.get(`${preCode}.serviceCode`).d('服务编码'),
      },
      {
        name: 'serviceName',
        type: 'string',
        label: intl.get(`${preCode}.serviceName`).d('服务名称'),
      },
    ],
    fields: [
      {
      ...
      },
      {
        ...
        type: 'string',
        label: intl.get(`${commonCode}.remark`).d('源服务编码'),
      },
      {
        name: 'whiteList',
        type: 'string',
  ...
  return {
    fields: [
      ...
      },
      {
        name: 'datasourceObj',
        type: 'object',
        label: intl.get(`${commonCode}.datasourceObj`).d('数据源'),
...
export { TransferDetailDS, DetailCreateFormDS };
