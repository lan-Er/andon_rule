 ...
 * @LastEditTime: 2020-06-28 15:54:14
 */
...
export default class QualityChart extends Component {
  ...
  handleChart() {
    ...
    dataList.forEach((data) => {
      ...
      newData.ds = new DataSet({
        autoCreate: true,
        fields: [{ name: 'percent', type: 'number', defaultValue: 20 }],
      });
  ...
  // 获取数据
  @Bind()
  async handleQualityData() {
    const { startTime, rateTime, sortFlag } = this.state;
    await qualityList({
      startDate: rateTime,
      endDate: startTime,
      rankingType: 'qualified',
  ...
  render() {
    ...
    return (
      <Fragment>
        <div className="supplier-quality-chart">
          <p className="title">合格率排行</p>
          ...
          <div className="chart-theader">
            ...
            <div className="chart-img" onClick={() => this.changeSortFlag()}>
              <span>合格率</span>
              <img src={sortFlag ? up : down} alt="" className="sort-style" />
            </div>
