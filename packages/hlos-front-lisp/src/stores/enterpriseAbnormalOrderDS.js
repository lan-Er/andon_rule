...
export const ListDS = () => {
  return {
    ...
    fields: [
      ...
      {
        name: 'receiveQty',
        label: intl.get(`${preCode}.receiveQty`).d('接收数量'), // 31-6
      ...
      },
      {
        ...
        transformResponse: (val, object) =>
          `${object.attribute8} ${moneyFormat(object.attribute7)}`,
      },
      {
        name: 'delayDays',
        label: intl.get(`${preCode}.delayDays`).d('延迟天数'), // 当前日期-12
        transformResponse: (val, object) =>
          `${Math.floor(
            (new Date().getTime() - new Date(object.attribute12 || 0).getTime()) /
              (24 * 3600 * 1000)
          )}天`,
      },
    ],
