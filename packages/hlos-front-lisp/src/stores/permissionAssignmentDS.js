...
const preCode = 'lisp.permissionAssignment.model';
const url = `${HLOS_LISP}/v1/datas/template`;
    ...
    fields: [
      {
        name: 'loginName',
        type: 'string',
        label: intl.get(`${preCode}.loginName`).d('账号'),
      },
      {
        name: 'realName',
        type: 'string',
        label: intl.get(`${preCode}.realName`).d('名称'),
      },
    ],
    transport: {
      read: ({ data }) => {
        return {
          url: `/iam/hzero/v1/0/users/paging/all`,
          data,
          method: 'GET',
        };
      },
    ...
    fields: [
      {
        name: 'meaning',
        type: 'string',
        label: intl.get(`${preCode}.meaning`).d('方案包'),
      },
      {
        name: 'value',
        type: 'string',
        label: intl.get(`${preCode}.value`).d('编码'),
      },
    ],
    transport: {
      read: () => {
        return {
          url: `/hpfm/v1/${getCurrentOrganizationId()}/lovs/value?lovCode=LISP.FUNCTION_TYPE`,
          method: 'GET',
        };
      },
    },
...
export const PermissionAssignmentProvider = (props) => {
  const { children } = props;
  const listDS = useMemo(() => new DataSet(ListDS()), []);
  const userDS = useMemo(() => new DataSet(UserListDS()), []);
  const funcDS = useMemo(() => new DataSet(FuncListDS()), []);
