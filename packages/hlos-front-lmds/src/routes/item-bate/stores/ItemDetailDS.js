...
export default () => ({
  ...
  fields: [
    ...
    },
    {
      ...
      transformRequest: (value) => {
        const activeMeOuIndex = localStorage.getItem('lmds-item-bate-active-meou-index') || 0;
        return value[Number(activeMeOuIndex)];
      },
    },
    {
      name: 'meOuCode',
  ...
  transport: {
    ...
    submit: ({ data, params }) => {
      const _status = data[0]?.status || data[0]?._status || 'create';
      const itemAps = data[0].itemAps[0];
      const itemMe = data[0].itemMe[0];
      const itemScm = data[0].itemScm[0];
      const itemSop = data[0].itemSop[0];
      const itemWm = data[0].itemWm[0];

      return {
        url,
        data: {
          ...data[0],
          itemAps: itemAps ? { ...itemAps, _status } : itemAps,
          itemMe: itemMe ? { ...itemMe, _status } : itemMe,
          itemScm: itemScm ? { ...itemScm, _status } : itemScm,
          itemSop: itemSop ? { ...itemSop, _status } : itemSop,
          itemWm: itemWm ? { ...itemWm, _status } : itemWm,
          _status,
        },
        params,
        method: 'POST',
