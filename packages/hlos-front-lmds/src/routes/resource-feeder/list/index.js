...
const directory = 'resource-feeder';
const organizationId = getCurrentOrganizationId();
const { TabPane } = Tabs;

const Feeder = (props) => {
  const listDS = useDataSet(dsFactory, Feeder);
  ...
  const codeRender = ({ value, record }) => {
    return <a onClick={() => handleToDetailPage(record.data.feederId)}>{value}</a>;
  };
  ...
  function handleToDetailPage(val) {
    props.history.push({
      pathname: `/lmds/resource-feeder/detail/${val}`,
    });
  }
  ...
  return (
    <Fragment key="feeder">
      <Header title={intl.get(`${preCode}.view.title.list`).d('飞达')}>
        <Button icon="add" color="primary" onClick={handleAddLine}>
          {intl.get('hzero.common.button.create').d('新建')}
        </Button>
        <ExportButton
          reportCode={['LMDS.FEEDER']}
          exportTitle={intl.get(`${preCode}.buton.export`).d('飞达导出')}
        />
      </Header>
      <Content>
        <div style={{ display: 'flex', marginBottom: '10px' }}>
          <Form dataSet={listDS.queryDataSet} columns={4} style={{ flex: 'auto' }}>
            <TextField name="feederCode" />
            <TextField name="feederName" />
          </Form>
        ...
        <Tabs
          ...
          // className={styles['lmds-feeder-tabs']}
        >
          {tabsArr().map((tab) => (
            <TabPane
