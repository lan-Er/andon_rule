...
export default class DetailPage extends Component {
    ...
    if (this.isCreatePage) {
      if (state) {
        if (state.mode === 'copy') {
          dispatch(
            routerRedux.push({
              pathname: `/lmds/routing/detail/${state.routingId}`,
            })
          );
  ...
  async handleSubmit() {
    ...
    if (res && res.content && res.content[0]) {
      // 新建页面创建数据成功后跳转到详情页面
      const pathname = `/lmds/routing/detail/${res.content[0].routingId}`;
      dispatch(
        routerRedux.push({
          pathname,
        })
  ...
  handleCopy() {
    copyApi(this.detailDS.current.data.routingId).then((res) => {
      ...
      if (res && res.routingId) {
        const pathname = `/lmds/routing/create`;
        this.props.dispatch(
          routerRedux.push({
            pathname,
            state: {
