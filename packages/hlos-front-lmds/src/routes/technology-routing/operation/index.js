...
export default class OperationDetail extends Component {
  ...
  async handleSubmit() {
    ...
    if (res && res.content && res.content[0]) {
      // 新建页面创建数据成功后跳转到详情页面
      const pathname = `/lmds/routing/operation/${res.content[0].routingId}/${res.content[0].routingOperationId}`;
      dispatch(
        routerRedux.push({
          pathname,
        })
    ...
    return (
      <Fragment>
        <Header
          ...
          backPath={routingId ? `/lmds/routing/detail/${routingId}` : '/lmds/routing/detail'}
        >
