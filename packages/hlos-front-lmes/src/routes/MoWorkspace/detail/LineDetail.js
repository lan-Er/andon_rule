...
export default function LineDetail(props) {
  ...
  function handleRuleChange(record, ruleType) {
    if (record) {
      ...
      tableDS.children.moExecute.current.set(
        'executeRuleObject',
        record.ruleJson && JSON.parse(record.ruleJson)
      );
      ...
      props[`set${ruleType}RuleJson`](null);
    }
  }
  ...
  function mainFields() {
    return [
      ...
      <Lov name="ruleCodeObj" disabled={allDisabled} key="ruleCodeObj" />,
      <Switch
        name="mtoFlag"
