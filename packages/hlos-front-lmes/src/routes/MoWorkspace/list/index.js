...
const MoList = ({ listDS, history, dispatch, taskList, pagination }) => {
  ...
  useEffect(() => {
    ...
    if (location.pathname === '/lmes/mo-workspace/list' && myQuery) {
      handleSearch().then(() => {
  ...
  function queryFields() {
    return [
      ...
      <Switch name="topMoOnly" key="topMoOnly" />,
    ];
  }
  ...
  async function handleRelease() {
    ...
    if (checkValues.length > 20) {
      const res = await releaseMoSplit(selectedRowKeys);
      if (res) {
  ...
  function handleCheckboxChange(value, rowData) {
    ...
    if (value) {
      ...
      const idx = rowKeys.findIndex((i) => i === rowData.moId);
