...
const ShipNormalDetail = (props) => {
  ...
  async function handleSave() {
    ...
    if (getResponse(res)) {
      ...
      // const pathName = `/lwms/ship-platform/create/normal`;
  ...
  async function handleAddLine() {
    ...
    await LineDS.create(
      {
        shipLineNum: LineDS.records.length + 1,
  ...
  return (
    <Spin spinning={loading}>
      <Header title="创建普通发货单" backPath="/lwms/ship-platform/list" isChange={isDirty}>
