...
import { Lov, Form, TextField, Button, DataSet, Tooltip, DateTimePicker } from 'choerodon-ui/pro';
...
import { outSourceReturnDetailDS } from '@/stores/singleReturnPlatformDS';
import { prodIssueReturnSave } from '@/services/singleReturnService';
...
function RequestIssueReturnCreate({ dispatch, location }) {
  ...
  const detailDS = useMemo(() => new DataSet(outSourceReturnDetailDS()), []);
  /**
   * 设置默认组织
   */
  useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    // const validateLineValue = await detailDS.children.requestLineList.selected.validate(false, true);
    ...
    const params = {
      ...
      requestOperationType: 'OUT_SOURCE_RETURN',
      sourceDocId: data.issueRequestId,
      sourceDocNum: data.issueRequestNum,
        ...
        >
          <Fragment key="issue-request-platform-form">
            <Form dataSet={detailDS} columns={4}>
              ...
              <TextField name="outSourceOrder" disabled />
