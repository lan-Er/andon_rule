 ...
 * @LastEditTime: 2021-03-07 21:26:50
 */
/*
 * @Description:
 * @Author: Zhong Kailong
 * @LastEditTime: 2021-03-04 15:02:17
 */
...
import { itemLotTableDS, itemLotSearchDS } from '@/stores/itemDisassembleDS';
import styles from './index.less';

const tableDS = new DataSet(itemLotTableDS());
const queryDS = new DataSet(itemLotSearchDS());
...
function lotModal(props) {
  ...
  const handleAddLine = async () => {
    ...
    if (tableDS.data.length > 0) {
      for (let i = 0; i < tableDS.data.length; i++) {
        if (tableDS.data[i].get('lotNumber') === queryDS.current.get('lotNumber')) {
          addFlag = false;
          Modal.confirm({
            title: '批次号重复，是否删除？？',
          }).then((button) => {
    ...
    <>
      <Content>
        <div style={{ display: 'flex' }}>
          <Form dataSet={queryDS} columns={2} style={{ flex: 1 }} labelLayout="placeholder">
            <TextField name="quantity" />
            <TextField name="lotNumber" />
          </Form>
          <div style={{ flex: 1, flexShrink: 0, display: 'flex', alignItems: 'center' }}>
            <Button
              funcType="flat"
              color="primary"
              icon="add"
