...
export default function TabComponent({
  ...
  const mainColumns = [
    ...
    {
      title: intl.get(`${preCode}.DNCProgramCode`).d('DNC程序编码'),
      dataIndex: 'attributeString3',
      key: 'attributeString3',
      width: 200,
      resizable: true,
    },
    {
  ...
  function actionRender({ rowData }) {
    return [
      ...
      <a
        style={{ marginLeft: '10px' }}
        onClick={(e) =>
          onToDetailPage(e, `/lmes/neway/production-task/split-task/${rowData.taskId}`)
        }
      >
        {intl.get(`${preCode}.view.button.split`).d('任务拆分')}
      </a>,
    ];
  }
  ...
  function linkRenderer({ rowData }) {
    return (
      <a onClick={(e) => onToDetailPage(e, `/lmes/neway/production-task/detail/${rowData.taskId}`)}>
        {rowData.taskNum}
      </a>
    );
  }
