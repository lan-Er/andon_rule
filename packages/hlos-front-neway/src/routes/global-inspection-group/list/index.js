        ...
        command: ({ record }) => {
          return [
            <Button
              ...
              color="primary"
              funcType="flat"
              onClick={() => this.handleToDetail(`/lmds/neway/inspection-group/detail`, record)}
            >
      ...
      <Fragment>
        <Header title={intl.get(`${preCode}.view.title.inspectionGroup`).d('检验项目组')}>
          <Button
            icon="add"
            color="primary"
            onClick={() => this.handleCreate('/lmds/neway/inspection-group/create')}
          >
          ...
          <ExcelExport
            requestUrl={`${HLOS_LMDSS}/v1/${organizationId}/neway-inspection-groups/excel`}
