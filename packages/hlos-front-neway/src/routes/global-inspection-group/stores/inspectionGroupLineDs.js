...
const commonUrl = `${HLOS_LMDSS}/v1/${organizationId}/neway-inspection-group-lines`;
...
export default () => ({
  ...
  fields: [
    ...
    },
    {
      name: 'relatedGroup',
      type: 'intl',
      label: intl.get(`${preCode}.relatedGroup`).d('检验项目关联组'),
    },
    {
      name: 'inspectionClass',
      label: intl.get(`${preCode}.inspectionClass`).d('检验项目大类'),
      bind: 'inspectionItem.inspectionClass',
    },
    {
      name: 'standardValue2',
      label: intl.get(`${preCode}.nationalStandard`).d('国家标准'),
      bind: 'inspectionItem.standardValue2',
    },
    {
      name: 'standardValue3',
      label: intl.get(`${preCode}.enterpriseStandard`).d('企业标准'),
      bind: 'inspectionItem.standardValue3',
    },
    {
