...
export default class RoutingOperationList extends PureComponent {
  get columns() {
    ...
    return [
      ...
      { name: 'workCenterObj', editor: true, width: 120 },
      { name: 'workCenterDescription', width: 120 },
      {
      ...
      { name: 'inspectionRuleObj', editor: <Lov noCache />, width: 128 },
      {
        ...
        editor: (record) => {
          return (
            <TextField name="preSequenceNum" onChange={() => this.preSequenceNumOnChange(record)} />
          );
        },
        width: 128,
      },
      ...
      { name: 'attributeString3', editor: true, width: 128 },
          ...
          if (record.status !== 'add') {
            return [
              <Button
                ...
                color="primary"
                funcType="flat"
                onClick={() => this.handleToOperationPage('/lmds/neway/routing/operation', record)}
              >
   ...
   */
  @Bind()
  preSequenceNumOnChange(record) {
    const { detailDS } = this.props;
    const currentValue = record.get('preSequenceNum')?.split('#');
    if (currentValue) {
      const maxNum = Math.max(...currentValue);
      const targetIndex = detailDS.children.routingOperationList.toData().findIndex((item) => {
        return Number(item.sequenceNum) === maxNum;
      });
      if (targetIndex !== -1) {
        detailDS.children.routingOperationList.splice(targetIndex + 1, 0, record);
      } else {
        notification.error({
