...
export default () => ({
  ...
  fields: [
    ...
    },
    {
      ...
      type: 'string',
      bind: 'workCenterObj.prodLineId',
    },
    {
      name: 'attributeString2',
      type: 'string',
      bind: 'workCenterObj.prodLineCode',
    },
    {
      name: 'workCenterDescription',
      type: 'string',
      label: intl.get(`${preCode}.workCenterDescription`).d('工作中心描述'),
      bind: 'workCenterObj.resourceName',
    },
    {
    ...
    },
    {
      ...
      type: 'string',
      label: intl.get(`${preCode}.DNCProgramCode`).d('DNC程序编码'),
    },
    {
      name: 'startDate',
      type: 'date',
  ...
  events: {
    update: ({ name, record }) => {
      ...
      if (name === 'operationObj' && isEmpty(operationObj)) {
        record.set('routingOperationType', null);
        record.set('keyOperationFlag', null);
        record.set('collectorObj', null);
        record.set('executeRuleObj', null);
        record.set('inspectionRuleObj', null);
        record.set('dispatchRuleObj', null);
        record.set('packingRuleObj', null);
        record.set('reworkRuleObj', null);
      }
    },
