...
import { HLOS_LMES, HLOS_LMESS } from 'hlos-front/lib/utils/config';
...
const commonItemFields = [
  ...
  },
  {
    name: 'executableQty',
    type: 'number',
    label: intl.get(`${preCode}.executableQty`).d('返修合格数量'),
    min: 0,
  ...
  },
  {
    name: 'suggestQty',
    type: 'number',
    label: intl.get(`${preCode}.suggestQty`).d('已创建返修数量'),
    min: 0,
  },
  {
    name: 'processOkQty',
  ...
  {
    name: 'pendingQty',
    label: intl.get(`${preCode}.pendingQty`).d('出站数量'),
  },
  {
    name: 'wipQty',
    label: intl.get(`${preCode}.wipQty`).d('进站数量'),
  },
  {
    name: 'itemUsage',
      ...
      read: ({ data }) => {
        ...
        return {
          url: generateUrlWithGetParam(
            `${HLOS_LMESS}/v1/${organizationId}/neway-productive-tasks`,
            {
              taskStatusList,
            }
          ),
...
const itemLineDetailDS = () => ({
  ...
  fields: [
    ...
    },
    {
      ...
      type: 'string',
      label: intl.get(`${commonCode}.executeControlType`).d('限制类型'),
      lookupCode: common.completeControlType,
      required: true,
    },
    {
      name: 'executeControlValue',
  ...
  return {
    ...
    fields: [
      ...
      },
      {
        ...
        type: 'number',
        disabled: true,
        label: intl.get(`${preCode}.splittableQty`).d('任务可拆分数量'),
      },
    ],
    transport: {
      read: ({ data }) => {
        return {
          url: `${HLOS_LMESS}/v1/${organizationId}/neway-productive-tasks/select-task-split-header`,
          data,
          method: 'GET',
        };
      },
  ...
  return {
    ...
    fields: [
      ...
      },
      {
        name: 'pitStopQty',
        type: 'number',
        label: intl.get(`${preCode}.pitStopQty`).d('可进站数量'),
      },
      {
        name: 'processOkQty',
        type: 'number',
        label: intl.get(`${tpmCode}.processOkQty`).d('合格数量'),
      },
      {
        name: 'scrappedQty',
        type: 'number',
      ...
      },
      {
        name: 'reworkQty',
        type: 'number',
        label: intl.get(`${tpmCode}.reworkQty`).d('返修数量'),
      },
      {
        name: 'wipQty',
        type: 'number',
    ...
    transport: {
      read: ({ data }) => {
        return {
          url: `${HLOS_LMESS}/v1/${organizationId}/neway-productive-tasks/select-task-split-line`,
          data,
          method: 'GET',
        };
      },
