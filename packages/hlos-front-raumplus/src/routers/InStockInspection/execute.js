 ...
 * @LastEditTime: 2021-06-26 12:18:35
 */
...
function InStockExecute(props) {
  ...
  function handleClose(flag) {
    props.history.push({
      pathname: '/raumplus/in-stock-inspection',
      state: {
        flag,
      },
    });
    closeTab('/pub/raumplus/in-stock-inspection/execute');
  }
  ...
  async function handleSubmit(qcResult) {
    ...
    const judgeInspectionDocLotDtoList = detailsList.map((v) => ({
      ...v,
      qcResult: qcResult === 'FAILED' || qcResult === 'PASS' ? qcResult : v.curJudge.judgeResult,
      qcOkQty: qcResult === 'FAILED' ? 0 : v.qualifiedQty,
      qcNgQty: qcResult === 'FAILED' ? v.batchQty : v.unqualifiedQty,
      qcSecondOkQty: null,
      qcSecondNgQty: null,
      qcNgReasonId: null,
      qcNgReason: null,
    ...
    if (qcResult === 'FAILED') {
      curQcOk = 0;
      curQcNg = docQualifiedQty + docUnqualifiedQty;
    }
  ...
  function handleQualifiedQty(value) {
    ...
    if (detailsList.length && detailsList.length === 1) {
      // const cloneDetailList = [...detailsList];
      // if (cloneDetailList[0].batchQty >= parseFloat(value.toFixed(6))) {
      //   cloneDetailList[0].unqualifiedQty = parseFloat(
      //     (parseFloat(cloneDetailList[0].batchQty) - parseFloat(value)).toFixed(6)
      //   );
      //   cloneDetailList[0].qualifiedQty = parseFloat(value.toFixed(6));
      //   setDetailsList(cloneDetailList);
      //   setDocQualifiedQty(cloneDetailList[0].qualifiedQty);
      //   setDocUnqualifiedQty(cloneDetailList[0].unqualifiedQty);
      // }
      const cloneDetailList = [...detailsList];
      if (cloneDetailList[0].batchQty >= parseFloat(value.toFixed(6))) {
        cloneDetailList[0].unqualifiedQty = parseFloat(
        ...
        cloneDetailList[0].qualifiedQty =
          parseFloat(cloneDetailList[0].batchQty) - cloneDetailList[0].unqualifiedQty;
        setDetailsList(cloneDetailList);
        setDocQualifiedQty(cloneDetailList[0].qualifiedQty);
        setDocUnqualifiedQty(cloneDetailList[0].unqualifiedQty);
      }
  ...
  function handleUnqualifiedQty(value) {
    ...
    if (detailsList.length && detailsList.length === 1) {
      // const cloneDetailList = [...detailsList];
      // if (cloneDetailList[0].batchQty >= parseFloat(value.toFixed(6))) {
      //   cloneDetailList[0].unqualifiedQty = parseFloat(value.toFixed(6));
      //   cloneDetailList[0].qualifiedQty = parseFloat(
      //     (parseFloat(cloneDetailList[0].batchQty) - parseFloat(value)).toFixed(6)
      //   );
      //   setDetailsList(cloneDetailList);
      //   setDocQualifiedQty(cloneDetailList[0].qualifiedQty);
