  ...
  return (
    <Row className={styles['details-line']}>
      <Col span={10} className={styles.title}>
        ...
        <span>
          {tagCode || ''} {itemControlType === 'TAG' ? `(${batchQty})` : ''}
        </span>
        <span>
          {lotNumber || ''} {itemControlType === 'LOT' ? `(${batchQty})` : ''}
        </span>
        <span>
          {warehouseName && wmAreaName
            ? `${warehouseName}-${wmAreaName}`
            : warehouseName || wmAreaName}
        </span>
      </Col>
      <Col span={8} className={styles.total}>
        <span className={`${styles['input-qualified']} ${styles['total-span']}`}>
          <NumberField value={qualifiedQty || 0} onChange={onDetailsQualifiedQty} />
        </span>
        <span className={`${styles['input-unqualified']} ${styles['total-span']}`}>
          <NumberField value={unqualifiedQty || 0} onChange={onDetailsUnqualifiedQty} />
        </span>
      </Col>
