 ...
 * @Author: zmt
 * @LastEditTime: 2021-06-26 11:37:16
 */

import React from 'react';
import { connect } from 'dva';
import { routerRedux } from 'dva/router';
import { DataSet, Button, TextField, Modal, Row, Col, Spin, NumberField } from 'choerodon-ui/pro';
...
class Execute extends React.Component {
  ...
  handleCalculateInspectionQuantity = () => {
    ...
    if (headerData.samplingType === 'ALL') {
      ...
      this.setState({
        qualifiedQuantity: parseFloat(qualified.toFixed(6)),
        unqualifiedQuantity: parseFloat(unqualified.toFixed(6)),
      });
    }
  };
  ...
  handleCalculateDetailsQuantity = () => {
    ...
    this.setState({
      ...
      qualifiedQuantity: parseFloat(_qualifiedQuantity.toFixed(6)),
      unqualifiedQuantity: parseFloat(_unqualifiedQuantity.toFixed(6)),
    });
  };
        ...
        poorSampleModal.update({
          children: (
            <div>
              ...
              <div className={styles['modal-footer']}>
                <Button onClick={() => poorSampleModal.close()}>取消</Button>
                <Button
                  color="primary"
  ...
  handlePoorSampleConfirm = (curRecord) => {
    ...
    _allInspectionData = _allInspectionData.map((v, i) => {
      if (v.sampleNumber === curRecord.sampleNumber) {
        curIndex = i;
        return { ...v, activeReasonOptions: _badReasonList };
      }
      return { ...v };
    });
    this.setState({
  ...
  handleAddSample = async () => {
    ...
    const res = await handleInspectionDocLinesAdd({
      ...
      templateId: this.state.headerData.templateId,
    });
    this.setState({ loading: false });
    if (res && res.failed) {
      notification.error({
  ...
  handleDetailsQuantity = (value, type, detailsIndex) => {
    ...
    if (parseFloat(value) > parseFloat(record.batchQty)) {
      ...
      console.log('quantityRec', quantityRec);
    }
  ...
  handleClose = (flag) => {
    ...
    this.props.dispatch(
      routerRedux.push({
        pathname: '/raumplus/inspection-judgment',
        query: {
    ...
    closeTab('/pub/raumplus/inspection-judgment/execute');
  };
  ...
  handleSubmit = async (judgeResult) => {
    ...
    const judgeInspectionDocLotDtoList = detailList.map((v) => ({
      ...v,
      qcResult:
        judgeResult === 'FAILED' || judgeResult === 'PASS' ? judgeResult : v.curJudge.judgeResult,
      qcOkQty: judgeResult === 'FAILED' ? 0 : v.detailsQualifiedQuantity,
      qcNgQty: judgeResult === 'FAILED' ? v.batchQty : v.detailsUnqualifiedQuantity,
      qcSecondOkQty: null,
      qcSecondNgQty: null,
      qcNgReasonId: null,
      qcNgReason: null,
    ...
    const _badReasonList = badReasonList.map((v) => {
      const fileUrlList = [];
    ...
    allInspectionData.forEach((ele) => {
      ...
      let _submitLineList = ele.badReasonList.filter((v) => v.active);
    ...
    if (judgeResult === 'FAILED') {
      curQcOk = 0;
      curQcNg = qualifiedQuantity + unqualifiedQuantity;
    } else if (judgeResult === 'PASS') {
      curQcOk = qualifiedQuantity;
      curQcNg = unqualifiedQuantity;
    }
    // let curQcOk =
    //   qualifiedQuantity === 0 && unqualifiedQuantity === 0 && judgeResult === 'PASS'
    //     ? null
    //     : qualifiedQuantity;
    // let curQcNg =
    //   qualifiedQuantity === 0 && unqualifiedQuantity === 0 && judgeResult === 'PASS'
    ...
    const params = {
      ...
      qcOkQty: curQcOk, // 合格数量
      qcNgQty: curQcNg, // 不合格数量
      qcSecondOkQty: null,
      qcSecondNgQty: null,
      qcNgReasonId: null,
      qcNgReason: null,
  ...
  handleChangeSampleNumber = async () => {
    ...
    if (getResponse(resp)) {
      ...
      if (detailList && detailList.length === 1) {
        console.log('进来了');
        // const newDatailList = [...detailList];
        // newDatailList[0].detailsQualifiedQuantity = parseFloat(qualifiedSampleQty.toFixed(6));
        // newDatailList[0].detailsUnqualifiedQuantity = parseFloat(unqualifiedSampleQty.toFixed(6));
        // 计算行数据逻辑
        this.handleDetailsQuantity(parseFloat(unqualifiedSampleQty.toFixed(6)), 'unqualified', 0);
        this.setState({
  ...
  handleQualifiedQty = (value) => {
    const { editSampleNumber, detailList } = this.state;
    ...
    if (detailList.length && detailList.length === 1) {
      const cloneDetailList = [...detailList];
      if (cloneDetailList[0].batchQty >= parseFloat(value.toFixed(6))) {
        cloneDetailList[0].detailsQualifiedQuantity = parseFloat(value.toFixed(6));
        cloneDetailList[0].detailsUnqualifiedQuantity = parseFloat(
          (parseFloat(editSampleNumber) - parseFloat(value)).toFixed(6)
        );
        this.setState({
          detailList: cloneDetailList,
        });
      }
    }
  };
  ...
  handleUnqualifiedQty = (value) => {
    ...
    if (detailList.length && detailList.length === 1) {
      const cloneDetailList = [...detailList];
      if (cloneDetailList[0].batchQty >= parseFloat(value.toFixed(6))) {
        cloneDetailList[0].detailsUnqualifiedQuantity = parseFloat(value.toFixed(6));
        cloneDetailList[0].detailsQualifiedQuantity = parseFloat(
          (parseFloat(editSampleNumber) - parseFloat(value)).toFixed(6)
        );
        this.setState({
          detailList: cloneDetailList,
            ...
            <Tabs defaultActiveKey="1" animated={false} onChange={this.callback}>
              ...
              headerData.itemControlType !== 'QUANTITY' ? (
                <TabPane tab="明细" key="2">
                  {detailList.map((detailLine, detailsIndex) => (
                    <DetailsLine
                      key={uuidv4()}
                      {...detailLine}
