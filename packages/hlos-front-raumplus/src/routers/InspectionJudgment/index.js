 ...
 * @Description: 检验判定
 * @Author: zmt
 * @LastEditTime: 2021-05-12 18:01:36
 */
...
import { inspectionDocBatchJudge } from '../../services/inspectionJudgmentService.js';
...
class Inspection extends React.PureComponent {
  ...
  handleToDetails = (record) => {
    const pathname = '/pub/raumplus/inspection-judgment/execute';
  ...
  // 时间排序切换
  handleSortToggle = async (type) => {
    const { timeToggle, itemToggle, supplierToggle } = this.state;
    this.setState({ loading: true });
    const { ds } = this.props;
    if (type === 'creationDateSort') {
      ds.setQueryParameter('creationDateSort', Number(!timeToggle));
    } else if (type === 'itemSort') {
      ds.setQueryParameter('itemSort', Number(!itemToggle));
    } else if (type === 'supplierSort') {
      ds.setQueryParameter('supplierSort', Number(!supplierToggle));
    }
          ...
          <Row className={styles['content-header']}>
            <Col span={16} className={styles['ds-ai-center']}>
              <div
                ...
                onClick={() => this.handleSortToggle('creationDateSort')}
              >
                时间
                <img
              ...
              <div
                className={styles['ds-ai-center']}
                style={{ marginRight: '20px', cursor: 'pointer' }}
                onClick={() => this.handleSortToggle('itemSort')}
              >
                物料编码
                <img
                  style={{ marginLeft: '8px' }}
                  src={!this.state.itemToggle ? down : up}
                  alt="down-up"
                />
              </div>
              <div
                className={styles['ds-ai-center']}
                style={{ marginRight: '20px', cursor: 'pointer' }}
                onClick={() => this.handleSortToggle('supplierSort')}
              >
                供应商
                <img
                  style={{ marginLeft: '8px' }}
                  src={!this.state.supplierToggle ? down : up}
                  alt="down-up"
                />
              </div>
