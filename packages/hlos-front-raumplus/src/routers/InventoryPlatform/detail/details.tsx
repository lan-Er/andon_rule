...
function InventoryPlatformDetail(props) {
  ...
  const { organizationName, countNum, countType, countStatus, countStatusMeaning } =
    queryPara || {};
  useEffect(() => {
    ...
    ds.query();
    (ds as any).queryDataSet.getField('warehouseObj').setLovPara('organizationId', id);
  }, [ds, id]);
  ...
  const handleGoToProductionAdjustment = useCallback(() => {
    ...
    history.push(`/raumplus/inventory-platform/adjustment/${id}`);
  }, []);
  ...
  return (
    <Fragment>
      {businessKey === undefined ? (
        <Header title={generateTitle()} backPath="/raumplus/inventory-platform/list">
          <ExcelExport
