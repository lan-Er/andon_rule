...
const getHeadColumns = (router, dispatch) => {
  const handleGoEdit = (record) => {
    ...
    router.push(`/raumplus/inventory-platform/edit/${id}`, {
      _isCreate_: false,
  ...
  const handleGoDetail = (record) => {
    const id = record.get('countId');
    router.push(`/raumplus/inventory-platform/detail/${id}`);
    dispatch({
...
function InventoryPlatform(props) {
  ...
  // 跳转新建
  const handleCreate = () => {
    history.push('/raumplus/inventory-platform/create', {
      _isCreate_: true,
    });
  };
  ...
  function handleImport() {
    openTab({
      // 编码是后端给出的
      key: '/himp/commentImport/LWMSS.COUNT_ITEM',
      // key: '/himp/commentImport/LMES.MO',
      // MenuTab 的国际化必须使用 hzero.common 开头(或者其他公用多语言)
      title: intl.get('lmds.common.button.import').d('导入'),
      search: queryString.stringify({
        action: intl.get('lmds.common.button.import').d('导入'),
      }),
    });
  }
      ...
      <Content className={style['inventory-platform']}>
        ...
        {headId && (
          <>
            ...
            <Table columns={lineColumns} dataSet={lineDS} />
          </>
        )}
      </Content>
    </Fragment>
