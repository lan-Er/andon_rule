...
const ShipDetail = (props) => {
  ...
  async function handleSave() {
    ...
    if (getResponse(res)) {
      ...
      // const pathName = `/lwms/ship-platform/list`;
    ...
    if (getResponse(result) && !isEmpty(result.content)) {
      LineDS.forEach((record) => {
        record.set({
          ...
          shipRuleId: record.data.shipRuleId,
          shipRule: record.data.shipRule,
          packingRuleId: record.data.packingRuleId,
          packingRule: record.data.packingRule,
  ...
  return (
    <Spin spinning={loading}>
      <Header title="创建销售发货单" backPath="/lwms/ship-platform/list">
