  ...
  useEffect(() => {
    async function queryType() {
      ...
      if (typeRes && typeRes.content) {
        setReturnTypeObj(typeRes.content.find((i) => i.documentTypeCode === 'SO_SHIP_ORDER'));
        typeRes.content.map((i) => {
          const _i = i;
          if (_i.documentTypeCode === 'SO_SHIP_ORDER') {
            _i.active = true;
          }
          return _i;
        });
    ...
    <>
      ...
      <div className={styles['modal-footer']}>
        ...
        <Button
          ...
          onClick={() => onModalOk(returnTypeObj)}
        >
          确认
        </Button>
      </div>
