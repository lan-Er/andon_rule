...
export const normalHeadDS = () => ({
  ...
  fields: [
    ...
    },
    {
      ...
      defaultValue: 'STANDARD_SHIP_ORDER',
    },
  ],
  transport: {
    read: ({ data }) => {
      return {
        url: `${HLOS_LWMSS}/v1/${organizationId}/ship-order-headers/detail/pc`,
        data,
        method: 'GET',
      };
    },
...
export const normalLineDS = () => ({
  ...
  fields: [
    ...
    },
    {
      name: 'externalNum',
      type: 'string',
      label: intl.get(`${preCode}.externalNum`).d('母订单号'),
    },
    {
      name: 'promiseShipDate',
      type: 'date',
...
export const seniorSearchDS = () => ({
  ...
  fields: [
    ...
    },
    {
      name: 'soNum',
      type: 'string',
      label: intl.get(`${preCode}.soNum`).d('子订单号'),
    },
    {
      name: 'soLineNum',
      type: 'string',
    ...
    },
    {
      name: 'avaliableQty',
      type: 'string',
      label: intl.get(`${preCode}.avaliableQty`).d('库存量'),
    },
    {
      name: 'applyQty',
      type: 'number',
      label: intl.get(`${preCode}.currentShipmentQuantity`).d('本次发货数量'),
      required: true,
      step: 1,
      min: 0,
    },
  ...
  queryFields: [
    ...
    },
    {
      name: 'completedFlag',
      type: 'boolean',
      label: intl.get(`${preCode}.completedFlag`).d('只展示完工母订单'),
      labelWidth: '150',
      defaultValue: false,
    },
    {
      name: 'productFlag',
      type: 'string',
      label: intl.get(`${preCode}.productFlag`).d('订单类型'),
      defaultValue: '1',
    },
  ],
  transport: {
    read: ({ data }) => {
      return {
        url: `${HLOS_LWMSS}/v1/${organizationId}/ship-order-headers/so-query`,
        data: {
          ...data,
        },
        method: 'GET',
  ...
  events: {
    load: ({ dataSet }) => {
      dataSet.data.forEach((item) => {
        item.set('applyQty', item.get('demandQty'));
        item.set('promiseShipDate', moment(item.get('demandDate')).format('YYYY-MM-DD'));
      });
    },
  },
});
