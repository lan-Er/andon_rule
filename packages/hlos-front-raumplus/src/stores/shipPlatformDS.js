  ...
  return {
    queryFields: [
      ...
      },
      {
        ...
        type: 'string',
        label: intl.get(`${preCode}.attributeString1`).d('品牌'),
        lookupCode: lwmsShipPlatform.customerGroup,
      },
      {
        name: 'shippingMethod',
        type: 'string',
        label: intl.get(`${preCode}.shippingMethod`).d('发货方式'),
        lookupCode: lwmsShipPlatform.shippingMethod,
      },
      {
        name: 'attributeString2',
        type: 'string',
        label: intl.get(`${preCode}.attributeString2`).d('发运物流'),
        lookupCode: lwmsShipPlatform.attribute2,
      },
      {
        name: 'planShipDateStart',
        type: 'date',
        label: intl.get(`${preCode}.planShipDateStart`).d('计划发运时间>='),
        transformRequest: (val) => (val ? moment(val).format('YYYY-MM-DD HH:mm:ss') : null),
        dynamicProps: {
          max: ({ record }) => {
            if (!isEmpty(record.get('planShipDateEnd'))) {
              return 'planShipDateEnd';
            }
          },
        },
      },
      {
        name: 'planShipDateEnd',
        type: 'date',
        label: intl.get(`${preCode}.planShipDateEnd`).d('计划发运时间<='),
        min: 'planShipDateStart',
        transformRequest: (val) => (val ? moment(val).format('YYYY-MM-DD HH:mm:ss') : null),
      },
      {
        name: 'attributeString4',
        type: 'string',
        label: intl.get(`${preCode}.attributeString4`).d('关联单号'),
      },
    ],
    fields: [
      {
      ...
      {
        name: 'attributeString15',
        type: 'string',
        label: intl.get(`${preCode}.attributeString4`).d('已关联发货单'),
      },
      {
        name: 'sopOuName',
        type: 'string',
    ...
    transport: {
      read: ({ data }) => {
        ...
        return {
          url: generateUrlWithGetParam(`${HLOS_LWMSS}/v1/${organizationId}/ship-order-headers`, {
            statusList,
          }),
          data: {
            ...data,
  ...
  return {
    ...
    fields: [
      ...
      },
      {
        name: 'packageQty',
        type: 'object',
        label: intl.get(`${commonCode}.packageQty`).d('包件数量'),
      },
      {
        name: 'soNum',
        type: 'string',
    ...
    transport: {
      read: () => {
        return {
          url: `${HLOS_LWMSS}/v1/${organizationId}/raumplus/ship-order-lines`,
          method: 'GET',
        };
      },
    },
...
const relationModalDS = () => ({
  autoCreate: true,
  fields: [
    {
      name: 'consignee',
      type: 'string',
      label: intl.get(`${preCode}.consignee`).d('联系人'),
      required: true,
    },
    {
      name: 'consigneeAddress',
      type: 'string',
      label: intl.get(`${preCode}.consigneeAddress`).d('联系人地址'),
      required: true,
    },
  ],
});

export { headDS, lineDS, relationModalDS };
