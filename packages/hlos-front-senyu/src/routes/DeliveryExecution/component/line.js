...
import { getBasketList } from '@/services/deliveryExecutionService';
import styles from '../index.module.less';

const intlPrefix = 'senyu.purchaseReceipt';
 ...
 */
export const Line = connect(({ senyuDeliveryExecution: { submitData } }) => ({ submitData }))(
  ({ record, dispatch, submitData, currentProgress, detailInfo }) => {
    ...
    const handleOpenModal = () => {
      Modal.open({
        className: `${styles['senyu-delivery-execution-detail-modal']}`,
        key: modalKey,
        destroyOnClose: true,
        title:
          itemControlType === 'TAG'
        ...
        async onOk() {
          ...
          const arr = [];
          copiedSubmitData.map((item, index) => {
            if (item.receiveList) {
              item.receiveList.map((v, n) => {
                if (v.tagCodeType == 'CONTAINER') {
                  arr.push(v.tagCode);
                }
              });
            }
          ...
          if (arr.length > 0) {
            const res = await getBasketList({ tagCodes: arr.join(',') });
            copiedSubmitData.map((item, index) => {
              if (item.receiveList) {
                item.receiveList = item.receiveList.filter((v) => {
                  return v.tagCodeType !== 'CONTAINER';
                });
              }
              res.map((v) => {
                if (item.itemId === v.itemId) {
                  item.receiveList.push(v);
                }
              });
            });
          }
    ...
    //   return (
    //     <div className={styles['senyu-delivery-execute-content-more-modal']}>
    //       <Lov
    //         dataSet={shipLineDS}
    //         name="wmAreaObj"
