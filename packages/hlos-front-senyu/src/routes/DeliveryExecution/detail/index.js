...
const intlPrefix = 'senyu.senyuDeliveryExecution';
const commonPrefix = 'senyu.common';
const key = Modal.key();
...
function DeliveryExecution({ dispatch, submitData, match, history }) {
  ...
  const handleSearch = useCallback(async () => {
    ...
    dispatch({
      type: 'senyuDeliveryExecution/updateState',
      payload: {
  ...
  const handleExit = () => {
    Modal.confirm({
      ...
      if (button === 'ok') {
        ...
        history.push({
          pathname: '/senyu/delivery-execution/list',
          state: {
            headResultList: history.location.state.recList,
          },
        });
        closeTab(`/pub/senyu/delivery-execution-detail/${shipOrderId}`);
      }
    });
  };
  ...
  const handlePick = async () => {
    ...
    data.map((item) => {
      const ind = arr.findIndex((v) => {
        return v.shipLineId === item.shipLineId;
      });
      if (ind === -1) {
        arr.push(item);
      } else {
        arr[ind].receiveList.push(item.receiveList[0]);
      }
    });
  ...
  const handleSubmit = async () => {
    ...
    submitData.forEach((i) => {
      ...
      if (_i.receiveList) {
        _i.receiveList.forEach((el) => {
          ...
          if (_i.itemControlType === 'TAG') {
            _el.executedQty = _el.quantity;
            _el.pickedQty = _el.quantity;
          }
          if (_i.itemControlType === 'QUANTITY') {
    ...
    if (getResponse(res) && !res.failed) {
      ...
      arr.forEach((item) => {
        params.lineList.push({
          shipOrderLineId: item.shipOrderLineId,
          shippedQty: item.receiveList.reduce((p, n) => {
            return p + n.executedQty;
          }, 0),
        });
      });
      const resp = await updateOrberStatus(params);
      if (resp.errCode === 0) {
        notification.success();
      } else {
  ...
  const queryDetailList = useCallback(
    async (params) => {
      return dispatch({
        type: 'senyuDeliveryExecution/fetchDetail',
        payload: {
          shipOrderId,
          itemCode: '',
          page: -1,
      ...
      <div className={styles.content}>
        {isExpand ? (
          <div className={styles['order-detail']}>
            ...
            <div
              className={styles['senyu-delivery-execute-content-expand-icon']}
          ...
          <div
            className={styles['senyu-delivery-execute-content-bottom-left-hide']}
            onClick={handleChangeExpand}
          >
...
export default connect(({ senyuDeliveryExecution: { submitData } }) => ({ submitData }))(
