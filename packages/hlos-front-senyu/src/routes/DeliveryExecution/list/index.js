...
const DeliveryExecution = ({ dispatch, selectedIds, history }) => {
  ...
  useEffect(() => {
    ...
    if (
      (location.pathname === '/senyu/delivery-execution/list' && flag) ||
  ...
  const handleSearch = debounce(async (searchType) => {
    ...
    const list = await dispatch({
      type: 'senyuDeliveryExecution/fetchLines',
      payload,
    });
...
export default connect(({ senyuDeliveryExecution: { selectedIds } }) => ({ selectedIds }))(
