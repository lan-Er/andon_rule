...
import { DataSet, Table, CheckBox, TextField, Button, NumberField, Lov } from 'choerodon-ui/pro';
...
export default class WmArea extends Component {
  ...
  get columns() {
    return [
      ...
      { name: 'categoryObj', width: 150, editor: <Lov noCache /> },
      {
        ...
        width: 150,
        editor: <NumberField noCache onChange={this.handleNumberChange} />,
      },
   ...
   */
  @Bind()
  async handleNumberChange(data) {
    if (data <= 0) {
      notification.warning({ message: '货位容量应大于零' });
    }
  }
  ...
  async handleAddLine() {
    let addFlag = true;
    const data = this.tableDS.records;
    data.forEach((item) => {
      if (item.status === 'add') {
        addFlag = false;
      }
