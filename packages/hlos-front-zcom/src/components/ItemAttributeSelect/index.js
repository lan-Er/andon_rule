...
export default ({ itemId = '', itemDesc = '', data = {}, handleSure, disabled, warning, ds }) => {
  ...
  useEffect(() => {
    if (!itemId && ds && ds.current && ds.current.editing) {
      setTimeout(() => {
        ds.current.editing = false;
  ...
  async function handleOk() {
    ...
    if (ds && ds.current && ds.current.editing) {
      setTimeout(() => {
        ds.current.editing = false;
        ds.current.editing = true;
      });
