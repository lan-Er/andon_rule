...
function ZcomDeliveryApplyDetail({ match, dispatch, history, location, customizeTable }) {
  ...
  const handleSubmit = (checkFlag) => {
    ...
    return new Promise(async (resolve) => {
      ...
      submitDeliveryApply([params]).then(async (res) => {
        // if (res && !res.failed) {
        //   notification.success({
        //     message: '操作成功',
        //   });
  ...
  return (
    <Fragment>
      <Header title={getTitle()} backPath={state || '/zcom/delivery-order'}>
        {canEdit && (
          <>
            <Button onClick={() => handleSubmit(1)} color="primary">
              提交发货
            </Button>
