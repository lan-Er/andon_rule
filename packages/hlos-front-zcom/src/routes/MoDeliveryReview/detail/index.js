...
function ZcomDeliveryReviewDetail({ match, history }) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      opinionDS.setQueryParameter('deliveryOrderId', deliveryOrderId);
      LineDS.setQueryParameter('deliveryOrderId', deliveryOrderId);
      LogisticsDS.setQueryParameter('deliveryOrderId', deliveryOrderId);
      await HeadDS.query();
      setCanEdit(HeadDS.current.get('deliveryOrderStatus') === 'RELEASED');
  ...
  async function handleReview(type) {
    ...
    const approvalOpinion = opinionDS.current.get('approvalOpinion');
    if (approvalOpinion) {
    ...
    try {
      ...
      if (res && !res.failed) {
        ...
        const pathName = `/zcom/mo-delivery-review`;
  ...
  return (
    <Fragment>
      <Header
        ...
        backPath="/zcom/mo-delivery-review"
      >
      ...
      <Content>
        ...
        {headShow ? (
          <Form dataSet={HeadDS} columns={3} className="delivery-review-headInfo">
            ...
            <TextField name="receiveWarehouseName" key="receiveWarehouseName" disabled />
