...
export default ({ data = {}, handleSure, disabled, warning, ds, required = true }) => {
  ...
  useEffect(() => {
    ...
    // else {
    //   setData(ds.current.get('itemAttr'));
    // }
  }, [visible]);
  ...
  async function initForm() {
    ...
    formDS.current.set('itemDesc', ds.current.get('itemDesc'));
    attrData.forEach((v, index) => {
      ...
      {data && data.attributeValue1 ? (
        ...
        >
          <Popover content={content}>
            <div>{`${data.attributeDesc1}：${data.attributeValue1}`}</div>
          </Popover>
        </div>
      ) : (
